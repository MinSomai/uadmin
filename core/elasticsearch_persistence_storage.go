...
func (afo *ElasticSearchAdminFilterObjects) GetUadminDatabase() *UadminDatabase {
	return nil
}
...
func FilterElasticSearchModel(db IPersistenceStorage, schema1 *schema.Schema, filterString []string, model interface{}) {
	...
	for _, filter := range filterString {
		...
		context = operator.Build(nil, context, uadminField, filterValueTransformed, &SQLConditionBuilder{Type: "and"})
	}
}
