...
func main() {
	...
	if command == "prepare" {
		...
		// pre 1.16 with no modules: $GOPATH/src/github.com/sergeyglazyrindev/uadmin
		// 1.16 and above: $GOPATH/pkg/mod/github.com/sergeyglazyrindev/uadmin@$uadmin.Version
		...
		if _, err := os.Stat("go.mod"); err == nil {
			...
			if err == nil {
				for i := range fs.Require {
					if fs.Require[i].Mod.Path == "github.com/sergeyglazyrindev/uadmin" {
				...
				for i := range fs.Replace {
					if fs.Replace[i].Old.Path == "github.com/sergeyglazyrindev/uadmin" {
