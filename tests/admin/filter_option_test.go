...
func (suite *FilterOptionTestSuite) TestFilterOptionByYear() {
	userModel := &usermodels.User{Username: "admin", Email: "admin@example.com", FirstName: "firstname", LastName: "lastname", IsSuperUser: true}
	userModel.CreatedAt = time.Now().Add((-10 * 12 * 86400 * 30) * time.Second)
	uadminDatabase := interfaces.NewUadminDatabase()
	defer uadminDatabase.Close()
	uadminDatabase.Db.Create(userModel)
	userModel = &usermodels.User{Username: "admin1", Email: "admin1@example.com", FirstName: "firstname", LastName: "lastname", IsSuperUser: true}
	userModel.CreatedAt = time.Now().Add((-5 * 12 * 86400 * 30) * time.Second)
	uadminDatabase.Db.Create(userModel)
	userModel = &usermodels.User{Username: "admin2", Email: "admin2@example.com", FirstName: "firstname", LastName: "lastname", IsSuperUser: true}
	userModel.CreatedAt = time.Now().Add((-3 * 12 * 86400 * 30) * time.Second)
	uadminDatabase.Db.Create(userModel)
	userModel = &usermodels.User{Username: "admin3", Email: "admin3@example.com", FirstName: "firstname", LastName: "lastname", IsSuperUser: true}
	...
	newFilterOption := admin.NewFilterOption()
	newFilterOption.FetchOptions = func (afo *admin.AdminFilterObjects) []*admin.DisplayFilterOption {
		return admin.FetchOptionsFromGormModelFromDateTimeField(afo, "created_at")
	}
...
func (suite *FilterOptionTestSuite) TestFilterOptionByMonth() {
	...
	adminUserPage.FilterOptions = admin.NewFilterOptionsRegistry()
	adminUserPage.FilterOptions.AddFilterOption(newFilterOption)
	assert.True(suite.T(), len(adminUserPage.FetchFilterOptions()) > 0)
}
