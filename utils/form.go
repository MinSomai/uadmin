...
import (
	...
	"encoding/json"
	"fmt"
	approvalmodel "github.com/uadmin/uadmin/blueprint/approval/models"
...
func ProcessForm(modelName string, w http.ResponseWriter, r *http.Request, session *sessionmodel.Session, s *model.ModelSchema) reflect.Value { //(errMap map[string]string) {
	...
	m, ok := model.NewModel(modelName, true)
	if !ok {
	...
	if ID != 0 {
		database.Get(m.Interface(), "id = ?", ID)
	}

	if ID != 0 && preloaded.LogEdit {
		func() {
			log = &logmodel.Log{}
	...
	appList := []approvalmodel.Approval{}
	...
	for index := 0; index < t.NumField(); index++ {
		...
		if t.Field(index).Type.Kind() == reflect.Int {
			...
			if f.Approval && m.Elem().FieldByName(t.Field(index).Name).Int() != i && !perm.Approval {
				appList = append(appList, approvalmodel.Approval{
					ModelName:  modelName,
					ColumnName: f.Name,
			...
			val := ""
			if f.Type == preloaded.CMULTILINGUAL {
				...
				val = imageapi.ProcessUpload(r, f, modelName, session, s)
				if val == "" {
			...
			if f.Approval && m.Elem().FieldByName(t.Field(index).Name).String() != val && !perm.Approval {
				...
				newApproval := approvalmodel.Approval{}
				if f.Type == preloaded.CMULTILINGUAL {
					database.Get(&newApproval, "model_name = ? AND column_name = ? AND model_pk = ? AND approval_action = 0", modelName, f.Name, ID)
					...
					changed := false
					for _, lang := range langmodel.ActiveLangs {
						oldVal = translation.Translate(m.Elem().FieldByName(t.Field(index).Name).String(), lang.Code, false)
						newVal = translation.Translate(val, lang.Code, false)
						appVal = translation.Translate(newApproval.NewValue, lang.Code, false)
						if oldVal != newVal {
			...
			for _, m2mID := range _v {
				...
				database.Get(m2mItem.Addr().Interface(), "id="+fmt.Sprint(m2mID))
	...
	if !ok {
		database.Save(m.Elem().Addr().Interface())
	} else {
		saverI.Save()
	}
	...
	for _, approval := range appList {
		approval.ModelPK = database.GetID(m)
		approval.Save()
	}
	...
	newURL := strings.TrimPrefix(r.URL.Path, preloaded.RootURL)
	if r.FormValue("save") == "" {
		newURL = preloaded.RootURL + strings.Split(newURL, "/")[0]
	...
	if r.FormValue("save") == "another" {
		newURL = preloaded.RootURL + strings.Split(newURL, "/")[0] + "/new"
		http.Redirect(w, r, newURL, http.StatusSeeOther)
		return m
	}
	newURL = preloaded.RootURL + strings.Split(newURL, "/")[0] + "/" + fmt.Sprint(ID)
	http.Redirect(w, r, newURL, http.StatusSeeOther)
	return m
}
