...
func RenderHTML(w http.ResponseWriter, r *http.Request, path string, data interface{}, funcs ...interface{}) {
	...
	for i := range funcs {
		...
		if funcVal.Type().Kind() != reflect.Func {
			utils.Trail(utils.WARNING, "Interface passed to RenderHTML in funcs parameter should only be a function. Got (%s) in position %d", funcVal.Type().Kind(), i)
			continue
		}
	...
	if err != nil {
		...
		utils.Trail(utils.ERROR, "RenderHTML unable to parse %s. %s", path, err)
		return
	}
	...
	if err != nil {
		...
		utils.Trail(utils.ERROR, "Unable to render html template file (%s). %s", path, err)
	}
}
...
func RenderMultiHTML(w http.ResponseWriter, r *http.Request, path []string, data interface{}, funcs ...interface{}) {
	...
	if err != nil {
		w.WriteHeader(500)
		fmt.Fprint(w, err.Error())
		utils.Trail(utils.ERROR, "RenderMultiHTML unable to parse %v. %s", path, err)
		return
	}
	...
	if err != nil {
		...
		utils.Trail(utils.ERROR, "Unable to render html template file (%v). %s", path, err)
	}
}
