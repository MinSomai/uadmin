...
func GetFilter(r *http.Request, session *sessionmodel.Session, schema *model2.ModelSchema) (interface{}, []interface{}) {
	queryList := []string{}
	args := []interface{}{}
...
func exportHandler(w http.ResponseWriter, r *http.Request, session *sessionmodel.Session) {
	...
	schema, ok := model2.GetSchema(model)
	if !ok {
		PageErrorHandler(w, r, session)
		return
	}

	a, ok := model2.NewModelArray(modelName, false)
	if !ok {
		PageErrorHandler(w, r, session)
		return
	}
	m, _ := model2.NewModel(modelName, false)

	query, args := GetFilter(r, session, &schema)

	ap, ok := m.Interface().(utils.AdminPager)

	if ok {
		err = ap.AdminPage("id", true, 0, -1, a.Addr().Interface(), query, args...)
	} else {
		err = database.AdminPage("id", true, 0, -1, a.Addr().Interface(), query, args...)
	}
	...
	*/
	...
	for i := 0; i < a.Len(); i++ {
		...
		for c := 0; c < m.NumField(); c++ {
			...
			if schema.FieldByName(t.Field(c).Name).Type == preloaded.CDATE {
				...
				excelAdjustWidthHight(file, sheetName, colName, cellName, i+2, utils.GetString(a.Index(i).Field(c).Interface()))
	...
	fileName := authservices.GenerateBase64(24)
	for _, err = os.Stat("./media/export/" + fileName + ".xlsx"); os.IsExist(err); {
		fileName = authservices.GenerateBase64(24)
	}
	err = file.SaveAs("./media/export/" + fileName + ".xlsx")
	if err != nil {
		utils.Trail(utils.ERROR, "exportHandler unable to save file %s. %s", "./media/export/"+fileName+".xlsx", err)
	}
