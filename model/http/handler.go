...
func DAPIHandler(w http.ResponseWriter, r *http.Request, s *sessionmodel.Session) {
	// Parse the Form
	...
	r.URL.Path = strings.TrimPrefix(r.URL.Path, preloaded.RootURL+"api/d")
	r.URL.Path = strings.TrimPrefix(r.URL.Path, "/")

	urlParts := strings.Split(r.URL.Path, "/")

	ctx := context.WithValue(r.Context(), preloaded.CKey("dAPI"), true)
	r = r.WithContext(ctx)
