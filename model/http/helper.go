...
func getFilters(r *http.Request, params map[string]string, tableName string, schema *model.ModelSchema) (query string, args []interface{}) {
	qParts := []string{}
	args = []interface{}{}
...
func getQueryOrder(r *http.Request, params map[string]string) string {
	...
	for _, part := range orderParts {
		...
		if part[0] == '-' {
			if security.SQLInjection(r, part[1:], "") {
				continue
			}
			orderArray = append(orderArray, part[1:]+" desc")
		} else {
			if security.SQLInjection(r, part, "") {
				continue
			}
...
func getQueryLimit(r *http.Request, params map[string]string) string {
	...
	if security.SQLInjection(r, limitRaw, "") {
		return ""
	}
	return limitRaw
}
...
func getQueryOffset(r *http.Request, params map[string]string) string {
	...
	if security.SQLInjection(r, offsetRaw, "") {
		return ""
	}
	return offsetRaw
}
...
func getQueryJoin(r *http.Request, params map[string]string, tableName string) string {
	...
	for _, j := range joinList {
		...
		// Check for SQL injection
		if security.SQLInjection(r, toTable, "") ||
			security.SQLInjection(r, toColumn, "") ||
			security.SQLInjection(r, fromTable, "") ||
			security.SQLInjection(r, fromColumn, "") {
			continue
		}
	...
	if m2m == "1" {
		for _, f := range s.Fields {
			if f.Type == preloaded.CM2M {
				...
				m2mModelName[f.Name] = model.Schema[strings.ToLower(f.TypeName)].ModelName
			}
		}
	} else {
		...
		for _, part := range m2mList {
			...
			f := model.Schema[modelName].FieldByName(m2mParts[0])
			table2Schema := model.Schema[strings.ToLower(f.TypeName)]
...
func returnDAPIJSON(w http.ResponseWriter, r *http.Request, a map[string]interface{}, params map[string]string, command string, model interface{}) error {
	if params["$stat"] == "1" {
		start := r.Context().Value(preloaded.CKey("start"))
