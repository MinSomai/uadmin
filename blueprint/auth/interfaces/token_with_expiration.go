...
func (ap *TokenWithExpirationAuthProvider) GetUserFromRequest(c *gin.Context) core.IUser {
	...
	uadminDatabase.Db.Model(token).Where(&core.UserAuthToken{Token: header}).First(token)
	return &token.User
}
	...
	if db1.Error != nil {
		userAuthToken = &core.UserAuthToken{
			...
			SessionExpiresAt: sql.NullInt64{
				Int64: time.Now().UTC().Add(time.Duration(core.CurrentConfig.D.Uadmin.SessionDuration) * time.Second).Unix(),
				Valid: true,
			},
		}
