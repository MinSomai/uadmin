...
func initializeLanguage() {
	...
	langList := []langmodel.Language{}
	if database.Count(&langList, "") != 0 {
		// Setup Active languages
		langmodel.ActiveLangs = []langmodel.Language{}
		dialect1 := dialect.GetDialectForDb()
		dialect1.Equals("active", true)
		database.Filter(&langmodel.ActiveLangs, dialect1.ToString(), true)

		dialect1 = dialect.GetDialectForDb()
		dialect1.Equals("default", true)
		// Setup default language
		database.Get(&langmodel.DefaultLang, dialect1.ToString(), true)
		return
	}
	...
	for i, lang := range langs {
		...
		if l.Active {
			langmodel.ActiveLangs = append(langmodel.ActiveLangs, l)
		}
		if l.Default {
			langmodel.DefaultLang = l
		}
		utils.Trail(utils.WORKING, "Initializing Languages: [%s%d/%d%s]", colors.FGGreenB, i+1, len(langs), colors.FGNormal)
	}
	tx.Commit()
	utils.Trail(utils.OK, "Initializing Languages: [%s%d/%d%s]", colors.FGGreenB, len(langs), len(langs), colors.FGNormal)
}
...
func Translate(raw string, lang string, args ...bool) string {
	...
	dialect1.Equals("default", true)
	database.Get(&langmodel.DefaultLang, dialect1.ToString(), true)
	transtedStr = string(langParser[langmodel.DefaultLang.Code])
...
func Tf(path string, lang string, term string, args ...interface{}) string {
	...
	var buf []byte
	if lang == "" {
		lang = langmodel.DefaultLang.Code
	}
	...
	if preloaded.LangMapCache == nil {
		preloaded.LangMapCache = map[string][]byte{}
	}

	// Check if the translation is cached
	fileName := "./static/i18n/" + strings.ToLower(path) + "." + lang + ".json"
	var ok bool
	if preloaded.CacheTranslation {
		buf, ok = preloaded.LangMapCache[fileName]
		if !ok {
			if _, err = os.Stat(fileName); os.IsNotExist(err) {
				utils.Trail(utils.WARNING, "Unrecognized path (%s) - fileName:%s", path, fileName)
				return term
			}
			buf, err = ioutil.ReadFile(fileName)
			if err != nil {
				utils.Trail(utils.ERROR, "Unable to read language file (%s)", fileName)
				return term
			}
			preloaded.LangMapCache[fileName] = buf
		}
	} else {
		if _, err = os.Stat(fileName); os.IsNotExist(err) {
			utils.Trail(utils.WARNING, "Unrecognized path (%s) - fileName:%s", path, fileName)
	...
	if !isSchemaFile {
		...
		if err != nil {
			utils.Trail(utils.ERROR, "Unknown translation file format (%s)", path)
			return term
		}
...
func TranslateSchema(s *model.ModelSchema, lang string) {
	...
	//if err != nil {
	//	utils.Trail(utils.ERROR, "Invalid format for language file (%s)", fileName)
	//	return
	//}
...
func GetLanguage(r *http.Request) langmodel.Language {
	...
	for _, l := range langmodel.ActiveLangs {
